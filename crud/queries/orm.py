from database import async_engine, async_session_factory, create_minio_client, orm_logger
...
class AsyncORM:
    ...
    @classmethod
    async def first_insert_photo(cls, way_to_photo : str, way_to_metadata : str) -> int:
        """
        ...
        """
        metadata = cls.async_file_reader(way_to_metadata)
        photo = Photo(is_processed=False, **metadata)

        

        async with async_session_factory() as session:
            # Добавление метаданных в сессию реляционной БД
            session.add(photo) 
            photo_id = photo.id
