...
docker exec kafka-0 ./opt/bitnami/kafka/bin/kafka-topics.sh --alter --topic a_topic --config max.message.bytes=60000000 --bootstrap-server localhost:9092
docker exec kafka-0 ./opt/bitnami/kafka/bin/kafka-topics.sh --alter --topic b_topic --config max.message.bytes=60000000 --bootstrap-server localhost:9092
docker exec kafka-0 ./opt/bitnami/kafka/bin/kafka-topics.sh --alter --topic saver_topic --config max.message.bytes=60000000 --bootstrap-server localhost:9092
